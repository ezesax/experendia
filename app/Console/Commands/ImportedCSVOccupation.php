...
class ImportedCSVOccupation extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:occupation';
    ...
    {
        //
        $csv = Reader::createFromPath(storage_path('csv/ocupaciones.csv'), 'r');
        $csv->setDelimiter(",");
        ...
        foreach ($records as $record)
        {
            if(Tag::where('name', $record["Level 1"])->exists())
            {
                ...
                if(!$item->tag_type()->where('tag_type_id', 1)->exists())
                {
                    $item->tag_type()->syncWithoutDetaching([1]);
                }
            }else{
                $item = new Tag();
